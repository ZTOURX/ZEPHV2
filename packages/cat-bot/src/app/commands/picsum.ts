/**
 * /picsum — Random Picsum Photo
 *
 * Fetches a random photo from the Delirius API (api.delirius.store/random/picsum).
...
export const config: CommandConfig = {
  name: 'picsum',
  aliases: ['photo', 'randomphoto'] as string[],
  version: '1.0.0',
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Fetch a random photo from Picsum.',
  category: 'random',
  usage: '',
  cooldown: 5,
  hasPrefix: true,
...
async function sendRandomPicsum(ctx: AppCtx): Promise<void> {
  ...
  try {
    ...
    if (!url) throw new Error('Failed to build Picsum API URL.');
    ...
    const payload = {
      ...
      attachment: [{ name: `picsum.${ext}`, stream: imageBuffer }],
